...
| 4 | Slither 无 High/Medium 未处理告警 | `slither . --config-file slither.config.json --fail-medium`（★**不能写 `--fail-on medium`**：CLI 无此选项，会以退出码 2 结束；带值的 `fail_on` 只能写在 `slither.config.json` 里。分级核查须用 `--json` 按 `impact` 统计，文本输出不含等级） |
