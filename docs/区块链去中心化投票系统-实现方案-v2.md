# 区块链去中心化投票系统 · 实现方案 v2.3（决策冻结 + 评审修订 + P5/P6 实现修正）
...
> v2.3：P6 自研不变量测试落地（路径 A），发现并修正「覆盖度陷阱」（F-24）
> 全部 24 项修订的清单与状态见 §15
...
| P6 | 不变量与随机测试（**路径 A 自研引擎**） | `test/invariant/{engine,world,invariants,actions}.ts` + 主测试 | ✅ **已达成**：INV-1~10 在 512×64（32,768 次动作）下无破坏；覆盖 887 个生命周期；阶段 0–5 均被访问；5 入口均成功、9 对抗性动作均被触发。✅ Slither 本机（wheels 解包）与 CI 均已跑通，并按 `--json` 分级查出 1 条 Medium（`unused-return`）后修复至 **0 High/Medium** | P5 |
...
| **F-24** | **中（测试有效性）** | **自研不变量测试首版存在「覆盖度陷阱」：32,768 次动作中 99.9% 发生在已封存的提案上，实际只覆盖 1 个提案生命周期，而测试全绿** | ✅ **已修（P6）**：新增 `spawnLifecycle` 动作重启生命周期 → 覆盖 **887** 个生命周期、空转降至 10.2%，**耗时几乎不变**。根因是流程终结后几乎所有动作权重归零，序列在终态空转。派生**硬约束 13**（必须统计并断言动作分布与状态覆盖），见 §16.5.2 |
...
**【已结清 · P6 实测】** 路径 A **能够**覆盖 INV-1（票数守恒）的规模化验证，
无需回退到路径 B。详见下节与 `P6-不变量测试-交付记录.md`。

#### 16.5.1 P6 落地结果（2026-09-18）

| 项 | 结果 |
| --- | --- |
| 引擎 | 自研，5 个文件约 1880 行（`test/invariant/{engine,world,invariants,actions}.ts` + 主测试） |
| 不变量 | INV-1 ~ INV-10（P5 新增 INV-10：投票路径事件不含发送者） |
| 动作集 | 19 个，覆盖 5 个状态变更入口 + 9 个对抗性动作（越权 / 伪造票据 / 重复提交 / 非法时序） |
| 实测（DoD 512×64） | **32,768 次动作 · 8 分 34 秒 · INV-1~INV-10 全程无破坏** |
| 覆盖度 | 887 个提案生命周期 · 6,145 个登记 · 3,652 张票 · 1,036 次接受揭示 · 492 次作废揭示 |
| 与 `forge invariant` 的差距 | 无收缩（shrinking）；无法构造畸形 calldata（ABI 层调用）。**已如实记入交付记录 §7** |

#### 16.5.2 ★ 本轮发现的「覆盖度陷阱」（新增硬约束 13）

**P6 首版引擎跑完 32,768 次动作、测试全绿，但实际只覆盖了 1 个提案的一次生命周期。**

统计暴露的问题：99.9% 的动作发生在已 `FINALIZED` 的提案上——流程终结后几乎所有动作
权重归零，只剩「重复调用」（必然 revert）与 `noop`，序列在终态上无限空转。
**这类缺陷不产生任何失败信号**：每条不变量在终态上本来就恒成立，测试自然全绿。

修正（新增 `spawnLifecycle` 动作重启生命周期）后的对比：
...
| 编号 | 要求 | 依据 |
| --- | --- | --- |
| **【硬约束 13】** | 随机/模糊测试必须**统计并断言动作分布与状态覆盖**（含流程生命周期数、各入口成功率、对抗性动作触发率），且断言值必须非零。禁止只断言「未发现不变量破坏」 | 否则覆盖率可以是宣称值的 1/887 而测试仍然全绿。本项目的落地形式：`p6.invariants.test.ts` 中断言「阶段 0–5 均被访问」「5 个入口均成功过」「9 个对抗性动作均被触发过」「生命周期数 > 3（默认）/ > 10（完整）」 |
