# 区块链去中心化投票系统 · 实现方案 v2.1（决策冻结 + 评审修订版）
...
| N7 | 主体仍为线上投票系统，原有架构不变 | ✅ 采纳；新增 Relayer 组件（必要条件），见 §5.3 |

### 0.3 Q1–Q12 冻结记录（2026-09-18 确认）

| # | 事项 | **冻结结论** |
| --- | --- | --- |
| Q1 | 是否引入 Relayer | **引入**（全匿名的必要条件） |
| Q2 | 是否接受揭示期渐进公开 | **接受** |
| Q3 | 揭示窗口时长 | **48h** |
...
| Q11 | 生产主网 | **Base** |
| Q12 | 揭示交易由谁广播 | **选民自行广播**；Relayer 仅用于投票期 |
...
| 交易提交 | **Relayer 代付 gas**（EIP-712 + ERC-2771），**仅服务投票期**；揭示交易由选民自行广播（Q12） |
...
| **修正方案** | **两段式准入**：<br/>**阶段①（登记期）**：管理员 `registerVoters(addresses[], commitments[])`，链上建立 `whitelist[addr] = commitment` 与 Merkle 名册根。此阶段「谁有资格」公开，符合 N1/N2。<br/>**阶段②（投票期）**：选民在本地生成 ZK 证明，交易由 **Relayer** 提交。链上只出现 `nullifier` 与 `ballotCommitment`，不出现发送者身份。非名册成员**无法构造有效证明**，准入拦截自动生效 |
| **信任模型** | 白名单地址持有者即使知道全部名单，也无法从链上数据反推任何一张票归属。Relayer 只广播，看不到选票内容（见 §3.2 表） |
| **代价** | 新增 Relayer 组件（约 1 个服务 + 1 个中继合约）。若**不引入 Relayer**，隐身份降级为「谁投了公开、投了什么保密」 |
...
| N2 白名单拦截 | ZK 成员证明 + 交易经 Relayer 提交 | 非名册 commitment 的 proof 100% revert |
...
| Relayer | 交易 calldata 中的 `nullifier` 与 `commitment` | 选项明文（投票期为承诺） | 需信任其**不审查、不丢弃**交易 |
...
├── relayer/
│   └── MinimalForwarder.sol   # ERC-2771 中继（EIP-712 验签 + 代付）
├── libs/
...
| `MinimalForwarder` | ❌ 不可升级 | 中继逻辑简单，攻击面需最小 |
...
| 项 | 说明 |
| --- | --- |
| 必要性 | **是全匿名的必要条件**，不是性能优化。无 Relayer ⇒ `sender ↔ 选票` 关联成立 |
| 协议 | EIP-712 结构化签名 + ERC-2771 `MinimalForwarder` |
| 选民侧 | 本地生成 ZK proof → 构造 `castVote` calldata → 用钱包签 EIP-712 → 提交给 Relayer |
| Relayer 侧 | 验签（`ecrecover`）→ 校验 `nonce` 未用 → 代付 gas 广播 → 收取可选服务费（**默认 0**） |
| Relayer 能看到什么 | `nullifier`、`ballotCommitment`。**看不到选项明文**（投票期为承诺） |
| Relayer 看不到什么 | 真实身份、选项 |
| 免费额度 | **已冻结 Q5**：每提案每地址 ≤ 20 次，超出需自付 |
| 降级路径 | **已作废**（Q1 决定引入 Relayer，不做降级） |
| 服务范围 | **仅承担 `castVote` 的中继**。`reveal` 由选民自行广播（Q12），Relayer 不接触揭示明文（F-13） |

```solidity
// MinimalForwarder 核心（ERC-2771）
struct ForwardRequest {
    address from;      // 选民地址（EIP-712 签名者）
    address to;        // Proposal 合约
    uint256 value;     // 恒为 0
    uint256 gas;
    uint256 nonce;     // 每 from 独立递增
    uint256 deadline;
    bytes   data;      // castVote 的 calldata
}
...
> **【硬约束 4（评审修订 F-10）】** EIP-712 类型哈希必须覆盖 **全部** 字段，其中 `data` 须以 `keccak256(req.data)` 参与哈希；且 `execute()` 内必须校验 `req.deadline >= block.timestamp` 与 `nonce` 未使用。
> 缺少 `data` 绑定 ⇒ 同一签名可被重放至不同 calldata；缺少 `deadline` ⇒ 签名永久有效。二者均为高危。`to` 亦须参与哈希，防止跨提案重放。
...
struct ProposalConfig {
    ...
    address forwarder;       // MinimalForwarder
}
...
| P5 | `MinimalForwarder` 中继 | EIP-712 验签 + 代付 | 重放 nonce 必拒；`sender` 与选民解耦（链上无法关联） | P4 |
...
| F-10 | 中 | Forwarder 签名覆盖范围未定义，缺 `data`/`deadline` 绑定即高危 | ✅ 已修（硬约束 4 + R18） |
...
### 16.5 不变量测试的三条候选路径（P6 前须定案）

| 路径 | 说明 | 代价 |
| --- | --- | --- |
| A | Hardhat + 自研状态机随机测试 | 工作量最大，且断言能力弱于 Foundry `invariant` |
| B | 保留一台可访问 GitHub 的 CI 机器专跑 Foundry invariant | 需额外基础设施，形成双工具链维护成本 |
| C | 改用 Echidna / Medusa | 需 GitHub 访问；Windows 原生不可用 |

**【待确认】** 建议在 P1 期间决策，不晚于 P5 完成。
