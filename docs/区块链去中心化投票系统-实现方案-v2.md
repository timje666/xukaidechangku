...
| P6 | 不变量与随机测试（**路径 A 自研引擎**） | `test/invariant/{engine,world,invariants,actions}.ts` + 主测试 | ✅ **已达成**：INV-1~10 在 512×64（32,768 次动作）下无破坏；覆盖 887 个生命周期；阶段 0–5 均被访问；5 入口均成功、9 对抗性动作均被触发。⚠️ Slither 未执行（本机不可安装），移交 Linux CI | P5 |
